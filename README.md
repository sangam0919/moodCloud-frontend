...
---
### 최종 후기
앱과 웹을 모두 개발하는 과정에서 하나의 백엔드 로직이라도 **기기 환경(모바일 vs. 브라우저)**에 따라 다르게 동작해야 할 상황이 많았고, 이를 처리하기 위해 디바이스 환경을 고려한 분기 처리와 조건 설계의 중요성을 배웠습니다. 특히 멀티 디바이스·멀티 플랫폼 환경에 대한 이해를 바탕으로, 다양한 클라이언트 환경에서 안정적으로 작동할 수 있도록 백엔드 로직을 유연하게 설계하는 역량을 키웠습니다. 이 과정에서 직접 많은 자료를 찾아보며 다양한 플랫폼에 대응하는 백엔드 설계에 대한 실전 감각과 통합적인 이해를 넓힐 수 있었습니다.